					  ...
					  IN uint64 timeout)
{
	...
	printk(KERN_ALERT "[KERNEL] Message Received!");
	return RETURN_SUCCESS;
}
...
return_status do_authentication_via_spdm(void* spdm_context){
	uint8 digest_buffer[MAX_HASH_SIZE * MAX_SPDM_SLOT_COUNT];
	uint8 slot_id = 0;
	...
	if (slot_id != 0xFF) {
		status = spdm_get_certificate(spdm_context, slot_id, &cert_chain_size,
					      cert_chain);
		if (RETURN_ERROR(status)) {
	...
	status = spdm_challenge(spdm_context, slot_id, m_use_measurement_summary_hash_type,
				measurement_hash);
	if (RETURN_ERROR(status)) {
...
int e1000_open(struct net_device *netdev)
{
	...
	heartbeat_period = 0;
	global_session_id = 0;
	status = spdm_start_session(global_spdm_context, use_psk,
	...
	uint8 my_msg[] = {'h', 'e', 'l', 'l', 'o'};
	uint8 my_rsp[100];
	uintn size = sizeof(my_rsp);

	status = spdm_send_receive_data(global_spdm_context, &global_session_id, TRUE,
						my_msg,
						sizeof(my_msg),
						my_rsp,
						&size);
...
static int e1000_spdm_tx_map(struct e1000_adapter *adapter,
			struct e1000_tx_ring *tx_ring,
			...
			unsigned int mss, uint8_t spdm_msg_type)
{
	...
	volatile struct pci_dev *pdev = adapter->pdev;
	volatile struct e1000_tx_buffer *buffer_info;
	volatile unsigned int len = skb_headlen(skb);
	volatile unsigned int offset = 0, size, count = 0, i;
	volatile unsigned int f, bytecount, segs;
	...
	if (!spdm_msg_type)
	{
		...
		volatile uint8_t *enc_message;
		volatile uint32_t enc_offset, enc_batch_size;
		...
		{
			printk(KERN_INFO "[KERNEL] Linearizing skb\n");
...
static void e1000_spdm_tx_queue(struct e1000_adapter *adapter,
					...
					struct net_device *netdev, uint8_t spdm_msg_type)
{
	...
	count = e1000_spdm_tx_map(adapter, tx_ring, skb, first, max_per_txd,
...
static bool e1000_clean_rx_irq(struct e1000_adapter *adapter,
			       ...
			       int *work_done, int work_to_do)
{
	volatile struct net_device *netdev = adapter->netdev;
	volatile struct pci_dev *pdev = adapter->pdev;
	volatile struct e1000_rx_desc *rx_desc, *next_rxd;
	volatile struct e1000_rx_buffer *buffer_info, *next_buffer;
	volatile u32 length;
	volatile unsigned int i;
	volatile int cleaned_count = 0;
	volatile bool cleaned = false;
	volatile unsigned int total_rx_bytes = 0, total_rx_packets = 0;

	volatile uint8_t dec_message[0x1000];
	volatile uintn dec_message_size;
	volatile return_status ret_stat;
	...
	while (rx_desc->status & E1000_RXD_STAT_DD) {
		...
		{
			uint32 *session_id = kmalloc(sizeof(uint32), GFP_KERNEL); 
			*session_id = global_session_id;
			uint8_t is_app;

			dec_message_size = sizeof(dec_message);
			ret_stat = ((spdm_context_t *)global_spdm_context)->transport_decode_message(
					global_spdm_context, 
					&session_id, 
					&is_app, 
					FALSE, 
